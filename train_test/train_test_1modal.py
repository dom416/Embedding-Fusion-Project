...
from models.HFB_fusion_1modal import HFBSurv
...
def train(opt,train_data,test_data,device):
    ...
    random.seed(666)
    model = HFBSurv(1024, (48, 256), (20, 0, 20), (0.2, 0.3), 20, 0.1).to(device)
    model.to(device) 
...
def test_and_plot(opt,model, test_data, device):
    ...
    plot_survival_probabilities(all_survival_scores, all_censorships, all_event_times, all_case_ids, num_cases=20, output_dir=directory_path)
